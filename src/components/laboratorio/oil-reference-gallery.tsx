...
export default function OilReferenceGallery({ images }: { images: OilImage[] }) {
  ...
  return <>
    <div className="grid grid-cols-1 gap-4" aria-label="Galería de referencia">
      {images.map((image) => <button key={image.url} type="button" onClick={() => setSelectedImage(image)} aria-label={`Ampliar ${image.alt}`} className="block aspect-video overflow-hidden"><ResilientImage src={image.url} alt={image.alt} className="h-full w-full object-cover" /></button>)}
    </div>
    {selectedImage && <div role="dialog" aria-modal="true" aria-label={`Vista ampliada de ${selectedImage.alt}`} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 backdrop-blur-sm" onClick={() => setSelectedImage(null)}>
      ...
      <ResilientImage src={selectedImage.url} alt={selectedImage.alt} className="max-h-full max-w-full rounded border border-outline-variant/20 object-contain shadow-2xl" onClick={(event) => event.stopPropagation()} />
    </div>}
  </>;
}
