...
Bootstrap a Next.js 14 App Router monolith (TypeScript + Tailwind) on the VPS. Auth gates everything behind NextAuth.js (JWT session strategy, credentials + Google providers) with a file-based adapter for Fase 1 — MongoDB is deferred. The landing is a server-rendered `/` route composed of nine section components built sequentially with user validation. The blog lives behind auth as MDX/JSON static content rendered by server components. Comments are explicitly deferred and remain out of scope for Fase 1 by product decision. A single `middleware.ts` enforces protected routes and role-aware redirects.
...
**Rationale**: PM2 restarts and Nginx multi-worker demand durability without a DB. The store exposes an async `UserRepository` interface so Fase 2 swaps the implementation for Mongo without touching callers.
...
| `src/app/blog/page.tsx` | Create | Article listing (server) |
| `src/app/blog/[slug]/page.tsx` | Create | Article view (server) |
...
| `src/components/blog/article-card.tsx` | Create | Blog UI |
...
|-------|-------------|----------|
| Unit | `guard.canVisit`, first-user-admin rule, password length validation, article loader | Jest/Vitest, pure functions, mock repositories |
| Integration | Registration → login → session role, OAuth callback user creation, JSON store atomicity | Test API routes with supertest/`next/test` against temp data dir |
| E2E | Anonymous redirected from `/blog`, registered can `/blog`, first-user-admin, 9 sections render in order | Playwright (deferred if timeline; AGENTS.md allows UI-static exception only for pure visual sections) |
...
- [ ] If comments are ever reopened, define moderation and spam protection in a future change
