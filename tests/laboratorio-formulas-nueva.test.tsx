...
describe('/laboratorio/formulas/nueva page', () => {
  ...
  it('renders the new formula form for authenticated users', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'New formula', level: 1 })).toBeInTheDocument();
    ...
    expect(screen.getByRole('textbox', { name: /formula code/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /save formula/i })).toBeInTheDocument();
